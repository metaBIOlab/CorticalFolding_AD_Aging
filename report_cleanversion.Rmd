...
ggplot(data = dados_hemi_v1, aes(logExposedArea_age_decay, 1/2 * logAvgThickness_age_decay + logTotalArea_age_decay , color = Diagnostic, fill = Diagnostic, alpha = 0.4)) +
  ...
  geom_smooth(method = "lm", se = TRUE) +
  theme_pubr()+
  labs(y = "1/2 log(AvgThickness) + log(TotalArea) (after age correction)", x = "log(ExposedArea) (after age correction)" ) +
  scale_fill_manual(values=cbbPalette2) +
  scale_colour_manual(values=cbbPalette2) +
  guides(alpha = FALSE)
...
ggplot(
  ...
  geom_smooth(method = "lm", se = TRUE) + geom_hline(yintercept = 1.25, linetype = "dashed") +
  theme_pubr() +
  labs(y = "slope")+
  theme(legend.position = "none") +
  scale_fill_manual(values=cbbPalette2) +
...
ggplot(
  ...
  geom_point() +
  geom_errorbar(aes(ymin = estimate - std.error, ymax = estimate + std.error)) + stat_compare_means(method = "kruskal.test") +
  theme_pubr() +
  labs(y = "slope") +
  facet_wrap(ROI ~. ) +
  scale_fill_manual(values=cbbPalette2) +
...
ggplot(data = filter(lm_diagnostic_lobe_curv, term == "logExposedArea_age_decay"), aes(x = estimate, color = Diagnostic, fill = Diagnostic, alpha = 0.4)) +
  geom_density() +
...
ggplot(data = filter(lm_diagnostic_Agedecay_lobe, term == "logExposedArea_age_decay"), aes(x = estimate, color = Diagnostic, fill = Diagnostic, alpha = 0.4)) +
  geom_density() +
  geom_vline(xintercept = 1.25) +
  theme_pubr() +
  scale_fill_manual(values=cbbPalette2) +
...
```{r}
d <- density(filter(dados, ROI == "hemisphere", Diagnostic == "AD")$K)
valley <- optimize(approxfun(d$x,d$y), interval=c(-0.55,-0.53))$minimum

dados_lowAD <- filter(dados, ROI == "hemisphere", Diagnostic == "AD",  K > valley) %>% dplyr::select(c(SUBJ, Diagnostic, hemi))

dados_highAD <- filter(dados, ROI == "hemisphere", Diagnostic == "AD" ,  K < valley | K == valley )%>% dplyr::select(c(SUBJ, Diagnostic, hemi))

dados_highAD_all <- anti_join(dados, dados_lowAD)

dados_lowAD_all <- anti_join(dados, dados_highAD)

dados$ROI <- factor(dados$ROI, levels = c("hemisphere", "F", "O", "P", "T")) 
...
dados_adlevel <- rbind(dados, dados_highAD_all, dados_lowAD_all)
...
fig_K_spread
ggsave("fig_K_spread.pdf", plot= fig_K_spread, dpi=1200, width = 18, height = 22, units = "cm", device = "pdf")
```
...
cpK <-
  cutpointr(
    filter(dados_lowAD_all, Diagnostic == "AD" |
             Diagnostic == "CTL", ROI == "hemisphere"),
    K,
    Diagnostic,
    pos_class = "AD",
